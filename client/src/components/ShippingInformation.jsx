...
const ShippingInformation = () => {
	...
	const onSubmit = async (values) => {
		...
		const orderData = {
		  ...
		  shipping: shipping, // Választott szállítási mód
		};
	  
		try {
		  const token = localStorage.getItem('authToken');
	  
		  // Kérés a backendhez a SimplePay URL generálásához
		  ...
		  if (result.url) {
			// Ha sikerült az URL generálása, irányítjuk a felhasználót a SimplePay oldalra
			window.location.href = result.url;
		  } else {
			// Hibaüzenet kezelése, ha nem jött vissza URL
			console.error('Error generating payment URL:', result.message || 'Unknown error');
		  }
		} catch (error) {
		  // Hiba kezelése
